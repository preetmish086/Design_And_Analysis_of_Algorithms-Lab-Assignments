...
The algorithm first finds the smallest element in `A[1...n]`
and exchanges it with `A[1]`.

It then finds the smallest element in `A[2...n]` and exchanges
it with `A[2]`.

This process continues until the first `n-1` elements have been
processed.
...
At the beginning of iteration i, the subarray A[1...i-1]
contains the i-1 smallest elements of the original array
in sorted order.

In zero-based indexing used by the C program:

At the beginning of iteration i, A[0...i-1] contains the
i smallest elements in sorted order.

## 5. Proof of Correctness

The correctness of the algorithm is established using the three
...
Therefore, A[i] becomes the smallest element among all
remaining elements.
...
Since all other elements are already correctly positioned,
...
there is exactly one element remaining.

That element must be the largest remaining element and therefore
must already be in its correct position.
...
During iteration i, the algorithm compares the current minimum
...
However, Selection Sort still searches the entire remaining
unsorted portion during every iteration.
...
The best-case running time is therefore not asymptotically better
...
If the minimum element is already at the current position,
...
However, this does not change the asymptotic running time because
the minimum-search loop still performs all required comparisons.
...
The algorithm sorts the array in-place and requires only a constant
...
For an input of size n, the program calculates the theoretical
comparison count:
...
Thus, even for an already sorted array of five elements, the
algorithm performs ten comparisons.
...
Its loop invariant states that the already processed portion of the
array contains the smallest elements in sorted order.

The invariant is:

Initialized before the first iteration.
Maintained by placing the smallest remaining element at the
next position.
...
Thus, the best-case running time is not asymptotically better than
